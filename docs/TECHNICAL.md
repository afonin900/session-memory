...
### Полная индексация (`index --full` / без флагов)
...
### Будущее: MCP server (Фаза 4)

`core/` остаётся неизменным. Добавляется `mcp_server.py` — вторая тонкая обёртка:
...
@mcp.tool()
def search_sessions(query: str, mode: str = "keyword", project: str = None) -> list[dict]:
    ...

@mcp.tool()
def index_sessions(quick: bool = True) -> dict:
    ...
```

Таким образом, один и тот же `core/` обслуживает CLI (cli.py) и MCP (mcp_server.py).
...
`core/` уже готов к обёртке: `SearchEngine` и `Indexer` — независимые классы без зависимости от CLI. MCP server будет:

```
mcp_server.py
    +-- _get_store() / _get_vector_store()  (те же фабрики что в cli.py)
    +-- SearchEngine.search() -> list[SessionFragment]
    +-- Indexer.index_incremental()
```

Никаких изменений в `core/`, `storage/`, `parsers/` не потребуется.
